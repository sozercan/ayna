...
public final class SoundEngine {
    ...
    public enum Sound: String, CaseIterable {
        ...
        var systemSoundID: SystemSoundID {
            ...
            case .messageSent:
                ...
                return 1004  // Mail Sent
            case .messageReceived:
                // Message received - gentle notification
                return 1007  // SMS Received (subtle tri-tone)
            case .error:
                // Error - system alert
                return 1073  // Shake (error-like)
            case .newConversation:
                // New conversation - light tap
                return 1104  // Begin recording (subtle)
            case .success:
                // Success - confirmation
                return 1057  // Photo shutter (subtle click)
            }
        }
        #endif
        ...
        var macOSSoundName: NSSound.Name? {
            switch self {
            case .messageSent:
                return NSSound.Name("Morse")  // Short, subtle
            case .messageReceived:
                return NSSound.Name("Tink")   // Gentle notification
            case .error:
                return NSSound.Name("Basso")  // Error sound
            case .newConversation:
                return NSSound.Name("Pop")    // Light tap
            case .success:
                return NSSound.Name("Purr")   // Confirmation
            }
        }
        #endif
    }
    ...
    private func playMacOSSound(_ sound: Sound) {
        guard let soundName = sound.macOSSoundName,
              let nsSound = NSSound(named: soundName) else {
            DiagnosticsLogger.log(
                .app,
                level: .error,
