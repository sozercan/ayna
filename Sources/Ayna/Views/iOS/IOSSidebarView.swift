...
//
    ...
    struct IOSSidebarView: View {
        ...
        var body: some View {
            conversationListView
                ...
                .onChange(of: conversationManager.conversations) { _, newValue in
                    performSearch(conversations: newValue)
                }
                .onDisappear {
                    searchTask?.cancel()
                }
        ...
        private func performSearch(
            ...
            let activeQuery = query ?? searchText

            guard !activeQuery.isEmpty else {
                conversationSections = ConversationTimelineGrouper.sections(from: sourceConversations)
                return
            }

            let immediateResults = sourceConversations.filter {
                $0.title.localizedCaseInsensitiveContains(activeQuery)
                    || ($0.metadataPreview?.localizedCaseInsensitiveContains(activeQuery) ?? false)
            }
            conversationSections = ConversationTimelineGrouper.sections(from: immediateResults)
            ...
            searchTask = Task { @MainActor in
                ...
                guard !Task.isCancelled else { return }
                conversationSections = ConversationTimelineGrouper.sections(from: results)
            }
        }
    }
