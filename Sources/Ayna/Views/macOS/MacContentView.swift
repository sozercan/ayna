    ...
    struct MacContentView: View {
        ...
        var body: some View {
            ZStack {
                ...
                if let errorMessage = conversationManager.persistenceErrorMessage {
                    VStack {
                        ErrorBannerView(
                            message: errorMessage,
                            onDismiss: { conversationManager.dismissPersistenceError() }
                        )
                        Spacer()
                    }
                    .padding(.top, 8)
                    .transition(.move(edge: .top).combined(with: .opacity))
                    .animation(.easeInOut(duration: 0.3), value: conversationManager.persistenceErrorMessage)
                } else if let errorMessage = deepLinkManager.errorMessage {
                    VStack {
                        ErrorBannerView(
                            message: errorMessage,
