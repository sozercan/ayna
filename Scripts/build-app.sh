#!/usr/bin/env bash
# Build script to create Ayna.app bundle from SwiftPM build output
...
# Configuration
CONF=${1:-release}
SIGNING_MODE=${AYNA_SIGNING:-dev}
APP_NAME="Ayna"
BUNDLE_ID="com.sertacozercan.ayna"
BUILD_DIR="$ROOT/.build/app"
APP_BUNDLE="$BUILD_DIR/$APP_NAME.app"

# Build for host architecture by default; allow overriding via ARCHES
ARCH_LIST=( ${ARCHES:-} )
...
<dict>
    ...
    <array>
        <dict>
            ...
            <string>com.sertacozercan.ayna</string>
    ...
    <array>
        <dict>
            ...
            <string>Ayna Conversation</string>
            <key>UTTypeIdentifier</key>
            <string>com.sertacozercan.ayna.conversation</string>
            <key>UTTypeTagSpecification</key>
            <dict>
                <key>public.filename-extension</key>
                <array>
                    <string>aynaconv</string>
                </array>
            </dict>
        </dict>
    </array>
    ...
    <!-- Build Metadata -->
    <key>AynaBuildTimestamp</key>
    <string>${BUILD_TIMESTAMP}</string>
    <key>AynaGitCommit</key>
...
ICON_SOURCE="$ROOT/Sources/Ayna/Resources/ayna.icon"
...
if [[ -f "$ROOT/Ayna.entitlements" ]]; then
  codesign "${CODESIGN_ARGS[@]}" --entitlements "$ROOT/Ayna.entitlements" "$APP_BUNDLE"
else
